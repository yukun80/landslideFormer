...
class Trainer(DefaultTrainer):
    ...
    def test_with_TTA(cls, cfg, model):
        """在训练结束时使用测试时增强（Test-Time Augmentation，简称TTA）进行模型评估。
        TTA是一种常用的提高模型性能的技术，它在测试时对输入数据进行增强（例如，翻转、裁剪等），
        然后将模型对所有增强版本的预测结果进行平均，以得到最终的预测结果。"""
        logger = logging.getLogger("detectron2.trainer")
        # In the end of training, run an evaluation with TTA.
        logger.info("Running inference with test-time augmentation ...")
        # SemanticSegmentorWithTTA是一个使用TTA的语义分割模型，它的定义在提供的类声明中。
        model = SemanticSegmentorWithTTA(cfg, model)
        # 为每个在配置中定义的语义测试数据集构建了一个评估器，并将这些评估器存储在evaluators列表中。每个评估器的输出文件夹设置为配置的输出目录下的"inference_TTA"子目录。
        evaluators = [
            cls.build_evaluator(cfg, name, output_folder=os.path.join(cfg.OUTPUT_DIR, "inference_TTA")) for name in cfg.DATASETS.TEST_SEMANTIC
        ]
        # 使用配置、TTA模型和评估器列表调用类方法test进行测试，并将结果存储在res中。
        res = cls.test(cfg, model, evaluators)
        # 最后，它将结果字典的键更新为在原键的基础上添加"_TTA"后缀，并以有序字典的形式返回结果。
    ...
    def test(cls, cfg, model, evaluators=None):
        ...
        if evaluators is not None:
            assert len(test_dataset) == len(evaluators), "{} != {}".format(len(test_dataset), len(evaluators))
...
def main(args):
    ...
    """
    register_coco_instances("CAS_landslide_coco_train", {}, "datasets/coco/annotations/instances_train2017.json", "datasets/coco/CAS_img_jpg")
    register_coco_instances("CAS_landslide_coco_val", {}, "datasets/coco/annotations/instances_val2017.json", "datasets/coco/CAS_img_jpg")
